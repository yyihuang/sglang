...
class TestFlashInferLinearGDNBackendCorrectness(CustomTestCase):
    ...
    def test_cake_exact_checkpoint_prefill_updates_tracked_state(self):
        cake_api = self._cake_api_or_skip()
        fixture = build_gdn_attention_fixture(
            self,
        ...
        batch.mamba_track_indices = torch.tensor(
            [0, 10, 0, 11, 12, 0, 0], dtype=torch.int64, device="cuda"
        )
        batch.mamba_track_seqlens = torch.tensor(
            [56, 100, 25, 120, 88, 80, 43], dtype=torch.int64, device="cuda"
        )
        ...
        cache_indices = _cache_indices(fixture)
        tracked_indices = batch.mamba_track_indices[batch.mamba_track_mask]
        dispatcher = fixture.backend.linear_attn_backend.kernel_dispatcher
        with mock.patch.object(
            cake_api,
            "load_cake_gdn_kernel",
        ...
        cake_final = cache.temporal[cache_indices].clone()
        cake_tracked = cache.temporal[tracked_indices].clone()

        self.assertGreater(load_kernel.call_count, 0)
        extend.assert_called_once()
        self.assertEqual(extend.call_args.kwargs["num_state_checkpoints"], 3)
        self.assertEqual(
            extend.call_args.kwargs["state_checkpoint_every_n_tokens"], 64
        )
        self.assertEqual(
            extend.call_args.kwargs["state_checkpoint_cu_starts_i32"].dtype,
            torch.int32,
        )

        cache.conv[0].copy_(initial_conv)
        cache.temporal.copy_(initial_ssm)
        dispatcher.extend_kernel = TritonGDNKernel()
        triton_output = run_gdn_fixture_eager(fixture)
        triton_final = cache.temporal[cache_indices]
        triton_tracked = cache.temporal[tracked_indices]

        torch.testing.assert_close(
            cake_output, triton_output, atol=1e-2, rtol=1e-2
        )
        torch.testing.assert_close(
            cake_final, triton_final, atol=1e-2, rtol=1e-2
        )
        torch.testing.assert_close(
            cake_tracked, triton_tracked, atol=1e-2, rtol=1e-2
        )
